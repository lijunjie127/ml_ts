...
# 1.读取csv格式数据
df = pd.read_csv('../raw_data/OTS_statistics/ccgp_country_ptime_day_statistics.csv')
...
df = df.resample('m', closed='left').sum()
...
print(t)
t_diff7 = ts.diff(7)
t_diff7.dropna(inplace=True)
t7 = sm.tsa.stattools.adfuller(t_diff7)
print(t7)
plt.plot(t_diff7, label='t_diff7')
plt.show()
